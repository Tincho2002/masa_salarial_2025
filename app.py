...
def load_data(uploaded_file):
    try:
        ...
        st.error(f"ERROR CRÍTICO: No se pudo leer la hoja 'masa_salarial'. Mensaje: {e}")
        return pd.DataFrame()
...
if uploaded_file is not None:
    ...
    if df.empty:
        st.error("La carga de datos ha fallado o el archivo está vacío. El dashboard no puede continuar.")
        st.stop()
    ...
    if df_filtered.empty:
        ...
        concept_columns_to_pivot = ['Nómina General con Aportes', 'Antigüedad', 'Horas Extras', 'Cs. Sociales s/Remunerativos', 'Cargas Sociales Antigüedad', 'Cargas Sociales Horas Extras', 'Nómina General sin Aportes', 'Gratificación Única y Extraordinaria', 'Gastos de Representación', 'Gratificación por Antigüedad', 'Gratificación por Jubilación', 'SAC Horas Extras', 'Cargas Sociales SAC Hextras', 'SAC Pagado', 'Cargas Sociales s/SAC Pagado', 'Vacaciones Pagadas', 'Cargas Sociales s/Vac. Pagadas', 'Asignaciones Familiares 1.4.', 'Total Mensual']
        ...
        concept_columns_sipaf = ['Retribución Cargo 1.1.1', 'Antigüedad 1.1.3', 'Retribuciones Extraordinarias 1.3.1', 'Contribuciones Patronales 1.3.3', 'SAC 1.3.2', 'SAC 1.1.4', 'Contribuciones Patronales 1.1.6', 'Complementos 1.1.7', 'Asignaciones Familiares 1.4']
        sipaf_cols_present = [col for col in df_filtered.columns for expected in concept_columns_sipaf if expected.lower().replace(".", "") in col.lower().replace(".", "")]
        ...
        if sipaf_cols_present:
            ...
            pivot_table_sipaf.dropna(how='all', inplace=True)
