...
# --- Carga de datos con cache (sin cambios) ---
@st.cache_data
def load_data(url):
    try:
...
if df_filtered.empty:
    ...
    # ... (código de los 3 gráficos superiores sin cambios) ...
    ...
    with col_chart1:
        line_chart = alt.Chart(masa_mensual).mark_line(point=True, strokeWidth=3).encode(x=alt.X('Mes:N', sort=meses_ordenados, title='Mes'), y=alt.Y('Total Mensual:Q', title='Masa Salarial ($)', axis=alt.Axis(format='$,.0s'), scale=alt.Scale(domainMin=3000000000, domainMax=8000000000)), tooltip=[alt.Tooltip('Mes:N'), alt.Tooltip('Total Mensual:Q', format='$,.2f')]).properties(height=chart_height1, padding={'top': 25, 'left': 5, 'right': 5, 'bottom': 5}).configure_view(fill='transparent')
        st.altair_chart(line_chart, use_container_width=True)
    with col_table1:
        masa_mensual_display = masa_mensual[['Mes', 'Total Mensual']].copy()
        st.dataframe(masa_mensual_display.style.format({"Total Mensual": "${:,.2f}"}).set_properties(subset=["Total Mensual"], **{'text-align': 'right'}), hide_index=True, use_container_width=True, height=chart_height1 - 10)
    st.markdown("---")
    ...
    chart_height2 = 500
    with col_chart2:
        bar_chart = alt.Chart(gerencia_data).mark_bar().encode(x=alt.X('Total Mensual:Q', title='Masa Salarial ($)', axis=alt.Axis(format='$,.0s')), y=alt.Y('Gerencia:N', sort='-x', title=None, axis=alt.Axis(labelLimit=120)), tooltip=[alt.Tooltip('Gerencia:N', title='Gerencia'), alt.Tooltip('Total Mensual:Q', format='$,.2f')]).properties(height=chart_height2, padding={'top': 25, 'left': 5, 'right': 5, 'bottom': 5}).configure_view(fill='transparent')
        st.altair_chart(bar_chart, use_container_width=True)
    with col_table2:
        gerencia_data_display = gerencia_data.copy()
        st.dataframe(gerencia_data_display.style.format({"Total Mensual": "${:,.2f}"}).set_properties(subset=["Total Mensual"], **{'text-align': 'right'}), hide_index=True, use_container_width=True, height=chart_height2 - 10)
    st.markdown("---")
    ...
    # --- INICIA LA SOLUCIÓN CON PAGINACIÓN ---
    ...
    if not df_display.empty:
        ...
        # Aplicar el formato solo a la página visible (mucho más rápido y evita el bug)
        ...
        st.dataframe(
            df_page.style.format(format_mapper, na_rep="").set_properties(subset=columns_to_align_right, **{'text-align': 'right'}),
    ...
    if summary_df is not None:
        ...
        # ... (código del resumen anual sin cambios) ...
